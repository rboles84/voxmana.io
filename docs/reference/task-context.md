...
Direct relationships use task identity in a handoff filename/title, explicit Related/Dependency/Predecessor/Successor fields or sections, and card evidence links. Task IDs appearing only as examples in general evidence/decision prose remain incidental; explicit source links retain their separate one-hop evidence relationship. Incidental exact mentions are listed separately and do not become direct authority. Complete identity matching preserves suffixes and does not confuse VM-001 with VM-001A.
...
During the VM-639 Phase 4 cutover, VM-637 was separate unfinished work: its exact manual tracker entries and source/planning hashes were preserved externally, its untracked source files were kept byte-for-byte and uncommitted, and only its local views were rebuilt after integration. That preservation requirement prevented unrelated WIP from entering the Phase 4 change; it was not a permanent prohibition on committing the intake. After the completed cutover, separately Owner-authorized integration of VM-637 and its child backlog records may follow the normal workflow. Preserve the historical snapshot and decisions; committing the intake does not complete the parent plan, approve its proposed page changes, or admit a child implementation. VM-649 records the separately authorized intake integration.
