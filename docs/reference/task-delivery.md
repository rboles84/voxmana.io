...
The normal persisted-record path requires the strict committed card to declare `RobQA: PASS at <same SHA> ...` and `Candidate: <same SHA>`. For candidate checks only, an original durable QA artifact may supply the exact material SHA without a binding-only commit when committed Candidate and RobQA are both PENDING, Owner is PENDING, lifecycle is In Progress, and the reviewed SHA equals the clean current HEAD. The complete authentic QA, admission and execution checks still apply; output discloses `durable-qa` provenance. A stale or contradictory declared decision cannot use this path. Real later lifecycle updates still follow the existing card/evidence rules; integration requires declared Accepted and exact committed bindings. This path does not run QA or relabel self-review as independence. Preserve the [existing low-risk exception](../qa/RobQAPass.md#qa-execution-independence): an authenticated QA artifact may instead record `Execution: SAME-AGENT DISTINCT PHASE`, identical reviewer/implementer, `Independence required: no`, and its `Execution reason`. The acting agent must verify the risk decision and completed distinct phase; this is not an override for governance/shared/protected or stricter work. Risk/QA sufficiency stays with RobQA, not a pathname classifier. VM-641 requires SEPARATE.
...
The existing admission audit and Git report diff model inspect full history, net scope and rename endpoints. All post-candidate commits must be linear descendants. Runtime, policies, tools, tests, fixtures and otherwise unclassified paths cannot use the evidence exception. Uncertain content blocks.

For the task card, only lifecycle/delivery observations and checkbox results may differ; unchanged criterion wording, scope, decisions, dependencies and all other card content are compared. A Done relocation preserves its historical admission record and remains tracked through later evidence commits; current lifecycle status must agree with its canonical folder. A plan directly linked by the material candidate card and admitted in scope may change only its single Status field between exact existing lifecycle values, matching the current card, with every other byte unchanged and exact-delta content review. Unlinked plans, governing/decorated status text and other plan edits remain material/uncertain. Existing handoff prose cannot be rewritten; appended/new task evidence remains eligible for content review. Generated views must be fresh under the existing generator, whose bytes cannot change after the candidate.
...
- `route: {discoveryComplete: true, approvedBeforeAttempt: {read: [...], merge: [...]}, read, merge, expectedHeadGuard: true, writeAuthorized: true}`. Route values are `connector`, `gh` or `browser`. The approved arrays record interfaces repository governance identified for this operation before its first attempt; the selected routes must belong to those arrays. Failure does not expand the set. Guard/authorization are required before integration. Metadata does not guarantee every attempted write will succeed.
...
- `policy` always declares `required`, `basis: {authority, reference}` and `observed`. `authority` is `repository-rule`, `task-contract` or `owner-instruction`. The sole ordinary optional basis is `{authority: "repository-rule", reference: "docs/reference/workflow.md#main-protection-and-exceptions"}`; every other supported authority/reference makes the observation required. This mapping makes `required` authority-derived rather than an agent-selected response to unavailability. Missing, contradictory or unsupported state blocks.
- Observed policy uses `{required, basis, observed: true, allowed, source, requiredChecks: [...]}` with the actual source URL. Denial blocks. Unavailable policy uses `{required, basis, observed: false, unavailable: {route, reason}}`, where `route` was approved before the attempt. Optional unavailable policy continues with the repository-required `Deterministic Validation`; required unavailable policy blocks for Owner direction. Unavailability never changes `required`, expands approved routes, changes authentication or creates another verification layer.
...
The checker compares host files/commits to Git using the existing rename-aware machinery and validates live feature/main refs. Truncated files, missing blob observations, wrong heads, unknown CI, policy denial, required unavailable policy, unavailable guards and already-merged integration all block. If the main base advanced beyond locally verified integration input, reconcile before proceeding.
