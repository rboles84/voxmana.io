# VM-639 Independent RobQA — Task Context and Derived Views
...
Independently review the exact committed Phase 4 candidate, challenge recall and authority boundaries, run proportionate deterministic checks, preserve separate VM-637 work, and stop at engineering PASS and Owner Review. Applied the repo-local [RobDev skill](../../.agents/skills/robdev/SKILL.md), usage guide and [frozen gate](../dev/RobDevPass.md), plus [RobQA skill](../../.agents/skills/robqa/SKILL.md), usage guide and [frozen gate](../qa/RobQAPass.md). The [task-context contract](../reference/task-context.md), approved plan and explicit Owner corrections control behavior.

Reviewed AGENTS, gates, token policy, workflow admission/lifecycle and changed instructions, task-specific deep context, relevant board/index entries, VM-639 card and main implementation handoff, Phase 4 plan and Phase 3 closeout. Recent work integrated VM-638 admission and VM-632 routing. Inspected the actual full material scope: new reader/CLI/writer/tests, changed prompts/workflow/plan/package/CI, card/handoff, archive provenance and generated views. Archived old manual views received complete Git blob equality/hash checks; generated views received read-only regeneration and every-source coverage checks. Reviewed the complete correction diff separately.

## Classification and RobDev Packet

QA tier: QA-0 substantive governance with shared repository-tool behavior and bounded file-replacement risk. QA execution mode: SEPARATE. Reviewer vm639_independent_qa did not implement either candidate; shared behavior/governance requires independence, without expanding browser/certification breadth.

Changed behavior: focused/deep task recall, disclosed additional/incidental sources, tolerant legacy parsing and source-path selection, Git observations, deterministic tracker projections, archives, explicit paired writes/recovery, direct admission invocation and minimal conflicting-instruction cutover.
Owning authority/producer: cards and handoffs own authored observations; Git owns repository facts; unchanged strict admission owns permission. The tolerant reader projects information only. Existing exact-ID/section helpers and admission/report machinery are reused. Consumers inspected: six direct prompts, AGENTS preflight/handoff rules, workflow, CLI, both tracker views and CI.
...
- npm run test:admission: independently 43/43 PASS. npm run test:change-report: independently 2/2 PASS. Run on initial candidate; complete owning source/fixture bytes are Git-confirmed unchanged through correction, retaining applicability.
- npm run task -- indexes --check: PASS before evidence. Independent read-only generation confirms all 681 card and 1059 handoff source links are represented and both complete outputs match committed views after documented line-ending normalization.
- Informational examples: VM-632 resolves with three direct handoffs/plan; VM-044 lists both paths; VM-551 lists all nineteen paths without selecting; VM-639 focused/deep discloses scope and includes handoff/plan. Suffix identity is covered by focused fixtures.
- Exact archive verification: both committed archive blobs equal baseline originals byte-for-byte and match manifest revision/hash. Board 135713 bytes, SHA-256 87dfa535eb997790fcd817aabe7e7306cb1865fbfee134c048a3a12c4ca79128; index 539851 bytes, SHA-256 f6d8d9a9cef137f285c695f3190672b6b8c5753a4981fa236185efcc42d9a3bf. Archive-local attributes preserve bytes.
- Read-only original VM-637 preservation: both dirty trackers and all three untracked authored files match all five external vm639-preservation.json hashes. No VM-637 source entered the candidate.
- npm run lint:js: PASS. npm run validate:source-generated: PASS with two existing JESKAI/MARDU model-owned biological-prior warnings. Git baseline-to-candidate diff --check: PASS. Strict admission/helper/report sources, compatibility fixtures, frozen gates and product assets/data remain unchanged.
- Main-agent development evidence reviewed: existing deterministic lint:html, lint:js, validate:source-generated, test:parser, test:placement, test:maze-finds, test:deck-links, test:copy-boundaries and test:frontend-smoke PASS. Their untouched product contracts need no additional certification for this source-link correction.
- This report's Git-derived material count/paths must pass validate-change-report before delivery.

## Acceptance Criteria Dispositions

1. Progressive recall, raw access, old decisions, disclosure and ambiguity: PASS after the independent correction.
2. Shared tolerant reader/unchanged strict authority: PASS, source inspection and compatibility evidence.
3. Admission delegation/deferred stages: PASS, exact options/exits and no synthetic readiness.
4. Archives/index-only history: PASS, committed blob equality and provenance fixtures.
...
The main agent observed the generated index assigning this QA handoff to VM-632, VM-044, VM-551 and VM-637. Independent read-only reproduction confirmed directIds VM-639, VM-632, VM-044, VM-551, VM-637, although those extra IDs occur as test examples/preservation evidence. A minimal VM-639 handoff with Related card VM-639 and two fixture IDs under Tests Selected and Objective Evidence likewise produced directIds VM-639, VM-044, VM-632. Broad evidence-section prose was being treated as a declared task relationship. The same relationshipText-to-ID extraction in deep relatedCards could expand incidental examples into unrelated card bodies.
...
Reviewer authored only this handoff to bind independent evidence and the resolved finding to the corrected candidate. No material fix, generated-view write, staging or commit. Main agent owns subsequent evidence/card updates and index regeneration. VM-637/original dirty checkout, runtime/data, historical sources, frozen gates, credentials/settings and unrelated work remain untouched.
