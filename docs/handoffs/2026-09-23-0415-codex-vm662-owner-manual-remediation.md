...
### vm662r9 exact-candidate result

The exact material candidate is `27968d9c1d64a3ac6ff50e1cc8dd5330de5827b2`. Candidate-bound verification measured a `0px` border-center delta, `10px` top inset, and `44 x 44px` target on 1440px cards 1 and true right-edge card 5 and 1100px true right-edge card 8. All targets remained fully viewport-contained; real pointer travel, hit ownership, stationary geometry, exact-card add, no-modal behavior, and keyboard activation passed. The paired route cache key is `vm662r9`.

The controlled candidate run observed DOMContentLoaded/load `56.4/67.5 ms`, `45` resources, boot/24/48 DOM `499/660/804`, Search-to-first-24 `14.4 ms`, and no observed Long Tasks. Discovery inspection remained request-free until its explicit Search, and the frozen desktop/mobile Reading Finds state exposed only `['finds']` when the other sections were empty. These are raw local observations, not field or causal performance claims.
