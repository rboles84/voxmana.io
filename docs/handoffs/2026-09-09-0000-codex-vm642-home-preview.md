...
## Promotion — Owner accepts final Home copy

Owner input: “final copy looks fine to me.” This accepts preview material candidate 062e4c19902a4cc5a27bb65cfe5e5daaeabe375c after the explicit layout lock. Earlier Owner instruction authorized preserving current index.html as index_old.html and renaming indexWIP.html to index.html once liked. Proceeding with that mechanical promotion and guarded delivery; no second approval sought for unchanged copy/layout. No claim the Owner viewed the later promotion SHA.

RobDev: same admitted branch and baseline. Dedicated card-only amendments admitted both Home entry paths, the backup, three existing Home regression scripts and the final Done card path before editing them. Admission continue passed at 42796f9d25996818bcaf9b1a51ca0325d91ef492. Runtime scope: preserve original bytes, move approved preview to the root, restore production title, remove preview-only noindex/comment, and update the isolated CSS ownership comment without changing rules. Existing body, assets, routes, Guide, shared JS, source data and placement remain fixed.

### Test Strategist attributed handoff

Agent: Test Strategist /root/promotion_checks. Read-only audit of deterministic Home assertions for VM-642 promotion. Files reviewed: frontend-smoke, validate-frontend-html, check-page-backgrounds, route-metadata, copy-boundaries, VM620/621 Guide checks, preview HTML, Home runtime, package commands, validation/browser workflows and selected legacy browser selectors. Governing authority: RobQA skill/full pass and Test Strategist role. No files changed or tests executed by this specialist; this is test selection, not QA PASS.

Decision: QA-1 with bounded route promotion. SAME-AGENT DISTINCT PHASE is sufficient for parent post-commit verification. Replace obsolete live cards/Guide wrapper/chart expectations with ordered tool destinations, unique guided entry and labeled static Mardu dossier. Retain chart markup coverage against the exact original backup and retain unchanged legacy runtime/37-entry registry checks. Allow the retired philosophy strip to be absent while preserving its hidden invariant if present. Preserve shared HTML guards and stylesheet ordering; verify production metadata.

Selected checks: HTML lint, frontend smoke, route metadata, copy boundaries, VM620/621 Guide checks, background guard and diff hygiene; verify archive byte preservation, accepted-body/CSS parity and untouched protected owners. Required deterministic CI remains mandatory. Exact prose/art/provenance are checked through accepted-file parity, avoiding permanent tests duplicating copy strings.

Deferred harness maintenance: browser-smoke.mjs:546–547, visual-regression-home.mjs and lighthouse-home.mjs:154–156 still target the retired Mana Lens. They are not required CI and are not run or repaired in this promotion. The unchanged Home runtime still schedules legacy chart/registry loading before its missing-canvas guard returns; no zero-request claim. This known limitation does not change the accepted presentation or placement. Next agent: main implementation agent for committed candidate QA and integration.
